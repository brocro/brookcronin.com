...
- **Sybil attacks.** Originating paper: John R. Douceur, "The Sybil Attack," *International Workshop on Peer-to-Peer Systems (IPTPS)*, 2002. Overview: [Sybil attack](https://en.wikipedia.org/wiki/Sybil_attack), Wikipedia.
- **Semaphore.** A zero-knowledge group-membership and signalling protocol from Privacy & Scaling Explorations. [semaphore.pse.dev](https://semaphore.pse.dev/).
- **Sismo.** A ZK-based identity and selective-disclosure project; the team has shifted from ZK Badges to *Sismo Connect*, an SSO-style flow that aggregates and selectively discloses verifiable credentials. Background: [Sismo blog](https://sismo.mirror.xyz/).
- **Polygon ID, now Privado ID.** Open-source middleware for privacy-preserving credentials. [privado.id](https://www.privado.id/).
- **World ID.** Worldcoin's proof-of-personhood credential. [world.org/world-id](https://world.org/world-id).
- **Privacy Pass.** Originally a browser extension for anonymous authentication; now also an active IETF standards effort. Project: [privacypass.github.io](https://privacypass.github.io/). Standards: [IETF Privacy Pass WG](https://datatracker.ietf.org/wg/privacypass/about/).
